...
import Test.Hspec
import UseCases.ReservationUseCase (ReservationError (ReservationNotPossible))
...
runTryReservation :: ReservationMap -> Reservation -> Either ReservationError ReservationMap
  ...
  describe "Reservation Use Case (only pure code)" $ do
    ...
    it "reports an error if a reservation is not possible" $ do
      let badReservation = Reservation day "Gabriella. Miller" "gm@example.com" 17
          expectedError = ReservationNotPossible "Sorry, only 16 seats left on 2020-05-02"
      (runTryReservation initReservations badReservation) `shouldBe` (Left expectedError)
